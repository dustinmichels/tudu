...
onMounted(async () => {
	...
	try {
		...
		const initialListId = listStore.inboxList?.id ?? listStore.lists[0]?.id;
		if (initialListId && !listStore.activeListId && !listStore.activeView) {
			listStore.setActiveList(initialListId);
			await taskStore.fetchTasks(initialListId);
		}
	} catch (err) {
...
<template>
  <div class="flex flex-col h-screen w-screen overflow-hidden bg-white dark:bg-zinc-900 font-sans text-zinc-900 dark:text-zinc-100 antialiased">
    ...
    <div class="flex-1 flex min-h-0 w-full relative overflow-hidden">
      ...
      <div
        ...
        :class="[
          uiStore.isSidebarOpen ? 'translate-x-0 shadow-2xl md:shadow-none' : '-translate-x-full md:translate-x-0'
        ]"
      >
        <Sidebar class="w-full h-full" />
      </div>

      <!-- Center Pane: Main Task List (fills available space) -->
