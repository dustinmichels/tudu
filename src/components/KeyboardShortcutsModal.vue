...
const shortcutCategories: ShortcutCategory[] = [
	...
	},
	{
		...
		shortcuts: [
			{
				keys: ["Ctrl", "Tab"],
				description: "Move to next task in todo list",
			},
			{
				keys: ["Ctrl", "Shift", "Tab"],
				description: "Move backwards to previous task",
			},
			{
				keys: ["j", "or", "↓"],
    ...
    >
      ...
      <div class="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-zinc-800">
        <div class="flex items-center gap-2.5">
          ...
          <h2 id="shortcuts-modal-title" class="text-base font-semibold text-zinc-900 dark:text-zinc-100">
            Keyboard Shortcuts
          </h2>
        </div>
        <button
            ...
            >
              ...
              <div class="flex items-center gap-1 shrink-0">
                <template v-for="(key, index) in shortcut.keys" :key="index">
                  <span
                    v-if="key === 'or'"
                    class="text-[10px] text-zinc-400 font-normal px-0.5"
                  >
                    or
                  </span>
                  <kbd
      ...
      <!-- Footer -->
      <div class="px-5 py-3 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
        <span>Press <kbd class="px-1.5 py-0.5 font-mono text-[10px] rounded bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700">?</kbd> anytime to open</span>
        <button
          type="button"
          @click="closeModal"
